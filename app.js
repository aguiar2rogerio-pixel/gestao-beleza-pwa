...
async function seedData() {
  ...
  if (!services.length) await write('services', { id: crypto.randomUUID(), name: 'Corte', price: 35, commissionRate: 40, duration: 60, active: true });
  if (!professionals.length) await write('professionals', { id: crypto.randomUUID(), name: 'Profissional principal', active: true });
...
async function refreshConfig() {
  ...
  $('#appointment-service').innerHTML = activeServices.map((item) => `<option value="${item.id}" data-price="${item.price}" data-duration="${item.duration || 60}">${safe(item.name)} — ${money(item.price)}</option>`).join('');
  ...
  if (firstService) {
    $('#appointment-amount').value = Number(firstService.price).toFixed(2);
    $('#appointment-duration').value = String(firstService.duration || 60);
  }
}
...
async function renderAgenda(appointments) {
  ...
  const todays = appointments.filter((item) => item.dateKey === today).sort((a, b) => a.time.localeCompare(b.time));
  $('#agenda-date-title').textContent = new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' });
  ...
  if (!todays.length) {
    list.innerHTML = '<div class="empty-state agenda-empty"><span class="empty-icon">✦</span><strong>Agenda livre</strong><p>Adicione o primeiro horário do dia.</p><button class="primary-action" data-action="open-launch">+ Novo horário</button></div>';
    list.querySelector('[data-action="open-launch"]').addEventListener('click', () => showView('launch'));
    return;
  }
  list.innerHTML = todays.map((item) => {
...
function setAppointmentDefaults() {
  ...
  if (!date.value) date.value = dateKey();
  if (!time.value) {
    const now = new Date();
    const rounded = new Date(now.getTime() + 15 * 60 * 1000);
    rounded.setMinutes(Math.ceil(rounded.getMinutes() / 15) * 15, 0, 0);
    time.value = `${pad(rounded.getHours())}:${pad(rounded.getMinutes())}`;
  }
}
...
async function saveAppointment(event) {
  ...
  if (!date || !time || !clientName || !professionalId) return showToast('Preencha os dados do horário.');
  const appointments = await readAll('appointments');
  const conflict = appointments.find((item) => item.dateKey === date && item.time === time && item.professionalId === professionalId && !['cancelled', 'completed'].includes(item.status));
  if (conflict) return showToast('Já existe um horário nesse período para este profissional.');
  await write('appointments', { id: crypto.randomUUID(), dateKey: date, time, clientName, serviceId: $('#appointment-service').value, professionalId, duration, amount: Number($('#appointment-amount').value || 0), notes: $('#appointment-notes').value.trim(), status: 'scheduled', createdAt: todayISO(), updatedAt: todayISO() });
  event.target.reset();
...
async function openAppointment(id) {
  ...
  modal.innerHTML = `<div class="modal-card appointment-modal-card"><div class="modal-header"><div><p class="eyebrow">DETALHES DO HORÁRIO</p><h2>${safe(appointment.time)} · ${safe(appointment.clientName)}</h2></div><button class="icon-button" data-close-modal aria-label="Fechar">×</button></div><div class="appointment-detail"><div><span>Serviço</span><strong>${safe(service?.name || 'Serviço')}</strong></div><div><span>Profissional</span><strong>${safe(professional?.name || 'Profissional')}</strong></div><div><span>Valor</span><strong>${money(appointment.amount)}</strong></div><div><span>Status</span><strong>${safe(appointment.status)}</strong></div></div><p class="detail-note">${safe(appointment.notes || 'Sem observações.')}</p><div class="modal-actions">${canComplete ? '<button class="primary-action full" data-complete>Concluir atendimento</button><button class="secondary-action full" data-status="no_show">Marcar falta</button><button class="secondary-action full" data-status="cancelled">Cancelar horário</button>' : '<button class="secondary-action full" data-close-modal>Fechar</button>'}</div></div>`;
...
async function completeAppointment(appointment, service, modal) {
  ...
  const commissionAmount = amount * Number(service?.commissionRate || 0) / 100;
...
async function openCadastros() {
  ...
  modal.innerHTML = `<div class="modal-card" role="dialog" aria-modal="true"><div class="modal-header"><div><p class="eyebrow">CONFIGURAÇÃO</p><h2>Cadastros</h2></div><button class="icon-button" data-close-cadastros aria-label="Fechar">×</button></div><div class="cadastro-columns"><form id="new-service-form" class="form-card compact-form"><h3>Novo serviço</h3><label>Nome<input id="new-service-name" required placeholder="Ex.: Corte masculino"></label><label>Preço<input id="new-service-price" type="number" min="0" step="0.01" required placeholder="35,00"></label><label>Comissão (%)<input id="new-service-commission" type="number" min="0" max="100" step="1" value="40" required></label><label>Duração<select id="new-service-duration"><option value="30">30 min</option><option value="45">45 min</option><option value="60" selected>1 hora</option><option value="90">1h30</option><option value="120">2 horas</option></select></label><button class="primary-action full" type="submit">Adicionar serviço</button></form><form id="new-professional-form" class="form-card compact-form"><h3>Novo profissional</h3><label>Nome<input id="new-professional-name" required placeholder="Nome do profissional"></label><button class="primary-action full" type="submit">Adicionar profissional</button></form></div><div class="cadastro-list"><h3>Serviços cadastrados</h3><div>${services.map((item) => `<div class="cadastro-row"><span>${safe(item.name)}</span><small>${money(item.price)} · ${item.commissionRate || 0}% · ${item.duration || 60} min</small></div>`).join('') || '<p class="empty-state">Nenhum serviço cadastrado.</p>'}</div></div><div class="cadastro-list"><h3>Profissionais cadastrados</h3><div>${professionals.map((item) => `<div class="cadastro-row"><span>${safe(item.name)}</span><small>Ativo</small></div>`).join('') || '<p class="empty-state">Nenhum profissional cadastrado.</p>'}</div></div></div>`;
  ...
  modal.querySelector('#new-service-form').addEventListener('submit', async (event) => { event.preventDefault(); const name = $('#new-service-name').value.trim(); const price = Number($('#new-service-price').value || 0); const commissionRate = Number($('#new-service-commission').value || 0); const duration = Number($('#new-service-duration').value || 60); if (!name || price < 0 || commissionRate < 0 || commissionRate > 100) return showToast('Confira os dados do serviço.'); await write('services', { id: crypto.randomUUID(), name, price, commissionRate, duration, active: true }); await refreshConfig(); await openCadastros(); showToast('Serviço cadastrado.'); });
  modal.querySelector('#new-professional-form').addEventListener('submit', async (event) => { event.preventDefault(); const name = $('#new-professional-name').value.trim(); if (!name) return showToast('Informe o nome do profissional.'); await write('professionals', { id: crypto.randomUUID(), name, active: true }); await refreshConfig(); await openCadastros(); showToast('Profissional cadastrado.'); });
}
...
function bindEvents() {
  ...
  $$('[data-action="open-launch"]').forEach((button) => button.addEventListener('click', () => { setAppointmentDefaults(); showView('launch'); }));
